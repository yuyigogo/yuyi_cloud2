...
class CustomerAbnormalCountView(BaseView):
    def get(self, request, customer_id):
        ...
        data = {
            "customer_day_abnormal_info": normal_redis.hgetall(customer_day_abnormal_key),
            "customer_week_abnormal_info": normal_redis.hgetall(customer_week_abnormal_key),
            "customer_month_abnormal_info": normal_redis.hgetall(customer_month_abnormal_key),
        }
...
class SiteAbnormalCountView(BaseView):
    def get(self, request, site_id):
        ...
        data = {
            "site_day_abnormal_info": normal_redis.hgetall(site_day_abnormal_key),
            "site_week_abnormal_info": normal_redis.hgetall(site_week_abnormal_key),
            "site_month_abnormal_info": normal_redis.hgetall(site_month_abnormal_key),
        }
        return BaseResponse(data=data)
